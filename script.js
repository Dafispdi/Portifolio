...
// --- ToURL File Uploader (YilziDev API) ---
async function uploadToUrl() {
  const fileInput = document.getElementById('toUrlFile');
  const pv = document.getElementById('toUrlPreview');
  const lk = document.getElementById('toUrlLinks');

  const file = fileInput.files[0];
  if (!file) {
    showToast('⚠️ Pilih file dulu sebelum upload!', 'warning');
    return;
  }
  ...
  try {
    ...
    if (!data.share_link && !data.download_link) throw new Error('Response API tidak berisi link file.');
    ...
    pv.innerHTML = `
      ...
      ">
        ...
        <b>ID:</b> ${data.id || '-'}<br>
        <b>Download:</b><br>
        <a href="${data.download_link}" target="_blank" style="color:#38bdf8;">${data.download_link}</a><br><br>
        <b>Share:</b><br>
        <a href="${data.share_link}" target="_blank" style="color:#38bdf8;">${data.share_link}</a>
      </div>
    `;
    ...
    openBtn.className = 'dl';
    openBtn.innerHTML = '<i class="fa-solid fa-up-right-from-square"></i> Open File';
    openBtn.onclick = () => window.open(data.share_link, '_blank');

    lk.appendChild(copyBtn);
    lk.appendChild(openBtn);
    lk.style.display = 'flex';
  } catch (err) {
    pv.innerHTML = `<div class="error">❌ Gagal upload: ${err.message}</div>`;
  }
}
...
async function uploadWithRetry(file, maxRetries = 3) {
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      formData.append('uploader_name', 'KyxzanTzy'); // tambahkan nama kamu

      const response = await fetch('https://upload.yilzidev.tech/api/upload/file', {
        method: 'POST',
      ...
      if (response.status === 429) {
        ...
        showToast(`⏳ Rate limit, retry dalam ${delay / 1000}s`, 'warning');
      ...
      showToast(`⚠️ Gagal upload, mencoba lagi (${attempt}/${maxRetries})...`, 'warning');
...
function validateFile(file) {
  ...
  if (file.size > maxSize) throw new Error('File terlalu besar (maks 100MB)');
  if (!allowedTypes.some(t => file.type.startsWith(t))) throw new Error('Tipe file tidak diizinkan');
  return true;
}
